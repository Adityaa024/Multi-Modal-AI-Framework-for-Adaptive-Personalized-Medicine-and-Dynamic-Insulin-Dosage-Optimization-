...
## Research, Safety, and Clinical Validity Notes

- **Synthetic Evaluation Data:** The evaluation in this repository is currently based on a synthetic dataset generated by `scripts/generate_synthetic_t2d_data.py`. While the data generation incorporates physiologically plausible distributions, the clinical validity and generalizability of the models trained on this data remain theoretical.
- **Need for Clinical Validation:** The models and rules presented here *must* be validated on real-world, patient-level clinical data before any claims of clinical utility or safety can be made. 
- **Algorithm Development Only:** This repository is intended strictly for algorithm development, ablation studies, and evaluation reproducibility. Generated data and model outputs are synthetic and must not be interpreted as clinical advice.
- **In-text Citations:** (Note for authors) Please ensure appropriate in-text citations are added in the Introduction and Related Work sections of the manuscript to support claims regarding existing insulin-titration approaches.
